...
#include "neighbors/NeighborKernels.cuh"
...
void NeighborGrid::updateBoundarySize(const RunData& runData,
                                      ...
                                      uint32_t boundaryParticleCount)
{
    ...
    _fluid = createData(getCellCount(_grid.gridSize), fluidParticleCount);
    _boundary = createData(getCellCount(_grid.gridSize), boundaryParticleCount);
    updateFluid(runData, fluidPositions, fluidParticleCount);
    updateBoundary(runData, boundaryPositions, boundaryParticleCount);
}
