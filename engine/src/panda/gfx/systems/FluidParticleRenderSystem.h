...
{
public:
    FluidParticleRenderSystem(const Device& device, vk::RenderPass renderPass, size_t particleCount);
    PD_DELETE_ALL(FluidParticleRenderSystem);
    ~FluidParticleRenderSystem() noexcept;

    auto render(const FrameInfo& frameInfo) const -> void;
    [[nodiscard]] auto getImportedMemory() const -> sph::cuda::FluidParticlesDataImportedBuffer;
