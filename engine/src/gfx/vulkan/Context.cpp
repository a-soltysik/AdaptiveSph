    ...
    namespace panda::gfx::vulkan
{
          ...
          _window {window}
    {
        ...
        {
            _device =
                std::make_unique<Device>(*_instance, _surface, requiredDeviceExtensions, _requiredValidationLayers);
        }
        else
        {
    ...
    auto Context::makeFrame(Scene & scene) const -> void
    {
        ...
        const auto vertUbo =
            VertUbo {.projection = scene.getCamera().getProjection(), .view = scene.getCamera().getView()};
        ...
        fragUbo.activeDirectionalLights = scene.getLights().directionalLights.size();
        fragUbo.activePointLights = scene.getLights().pointLights.size();
        fragUbo.activeSpotLights = scene.getLights().spotLights.size();
